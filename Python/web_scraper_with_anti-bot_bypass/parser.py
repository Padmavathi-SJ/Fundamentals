...
def parse_products(html): #function to extract product data
    soup = BeautifulSoup(html, "lxml") # converts HTMl -> structured format, lxml -> dast parser
    products = [] # empty list to store products
    
    for item in soup.select(".product_pod"): #finds all elements with class .product
        name = item.select_one("h3 a")["title"].strip() # extract product name from elements with class .title and removes extra spaces(.strip())
