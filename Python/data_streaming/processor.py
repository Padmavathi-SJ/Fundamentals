...
def process(data):
    ...
    # calculate z-score
    current = data["temperature"] # current temperature reading
    if std_temp > 0:
    ...
    # determine alert status based on temperature
    if current > 100:
        status = "CRITICAL"
    elif current > 85:
        status = "WARNING"
    ...
    return {
        "avg": round(avg_temp, 2), # moving avg 
        "std": round(std_temp, 2),  # standard deviation
        "z": round(z_score, 2),    # Z-score
        "status": status  # Alert status string
    }